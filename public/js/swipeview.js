...
/*			if (!hasTouch) {
		...
		goToPage: function (p) {
			...
			this.slider.style.OTransitionDuration = '0ms';
		...
		_flipsetup: function(){
			...
			document.getElementById('swipeview-slider').addEventListener(navigatorsProperties[i],function(e)
					{
		...
		*/
		__pos: function (x) {
			...
			this.slider.style.webkitTransform = 'translate3d(' + x + 'px,0,0)';
			 ...
			 this.slider.style.OTransform= 'translate3d(' + x + 'px,0,0)';
		...
		__start: function (e) {
			//e.preventDefault();
            if (this.deactive) return;  ///a turn off switch added by jeffj
...
/*			if (newX > 0 || newX < this.maxX) {
		...
		__end: function (e) {
			...
			if (dist < this.snapThreshold) {
				...
				this.slider.style.OTransitionDuration = Math.floor(300 * dist / this.snapThreshold) + 'ms';
				this.__pos(-this.page * this.pageWidth);
				return;
			}
		...
		__checkPosition: function () {
			...
			this.slider.style.OTransitionDuration= Math.floor(500 * Math.abs(this.x - newX) / this.pageWidth) + 'ms';
